...
var competitiveMode =
  window.localStorage &&
  window.localStorage.getItem("talon-practice-challengemode") == "true";
if (!competitiveMode) {
...
var updateHighscores = function (lesson) {
  ...
  if (scoreList.length > 0) {
    highScoreText +=
      "<table><thead><tr><th>Score</th><th>Time</th><th>Date</th></tr></thead><tbody>";
    ...
    scoreList.forEach(function (scoreRow) {
      highScoreText +=
        ...
        scoreRow.score +
        "</td><td>" +
        formatTimer(scoreRow.time) +
        "</td><td>On today!</td></tr>";
    });
    highScoreText += "</tbody>";
  } else {
  ...
  if (gameStarted) {
    ...
    if (competitiveMode) {
      ...
      if (changes > currentLesson.expectedChanges) {
        ...
        scoreExplanation +=
          'You used more steps than neccesary: <span style="color: red;">-50</span> for each extra step (' +
          (changes - currentLesson.expectedChanges) +
          ")<br/>";
      }
...
var dynamicKeypress = function (event) {
  if (event.keyCode === 37 || event.keyCode === 39) {
    var lessonList = Array.prototype.slice.call(
